...
#       def validate(
#             %{
#               # Each component needs a ref. This will be used for addressing (sending the component messages)
#               ref: _ref,
#               # Flamelex GUI components all have a defined %ScenicWidgets.Core.Structs.Frame{}
#               frame: %ScenicWidgets.Core.Structs.Frame{} = _f,
#               # `state` is the holder for whatever data it is which defines the internal state of the component (usually a map)
#               state: _x
#             } = data
#           ) do
#         {:ok, data}
...
#           ) do
#         opts = [
#           # we need to register the component with a name in Scenic, by passing it in as an option
#           id: r
#         ]

#         # REMINDER: `args` will in turn be passed into `validate/1, and if
#         #          that succeeds, on to init/3 (as this is the way Scenic works)
#         # REMINDER: Under the hood, this is calling Scenic.Component.add_to_graph/3
#         graph |> add_to_graph(args, opts)
#       end
