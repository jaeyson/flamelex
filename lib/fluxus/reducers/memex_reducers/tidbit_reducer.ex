defmodule Memelex.Fluxus.Reducers.TidbitReducer do
  ...
  def process(
        %{story_river: %{focussed_tidbit: t_uuid}} = radix_state,
        {:open_tidbit, %{uuid: t_uuid}}
      ) do
  ...
  def process(radix_state, {:set_gui_mode, new_mode, %{tidbit_uuid: t_uuid}}) do
    updated_tidbits =
      ...
      |> Enum.map(fn
        ...
        other_tidbit ->
          # make no changes to other TidBits...
          other_tidbit
      end)
  ...
  def process(radix_state, {:move_tidbit_focus, tidbit, new_focus})
      when new_focus in [:title, :body] do
    new_radix_state =
      radix_state
