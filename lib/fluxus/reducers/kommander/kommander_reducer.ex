defmodule Flamelex.Fluxus.Reducers.Kommander do
  ...
  def process(
        %{kommander: %{buffer: %{cursors: [old_cursor]} = old_k_buf}} = radix_state,
        {:modify_kommander, {:insert, text, :at_cursor}}
      ) do
    new_cursor =
    ...
    new_radix_state = radix_state |> put_in([:kommander, :buffer], new_k_buf)
