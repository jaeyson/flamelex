defmodule Flamelex.GUI.Component.Renseijin do
  ...
  def init(%Scenic.Scene{} = scene, {%Frame{} = frame, %State{} = state}, opts) do
  ...
  def handle_cast(:start_animation, %{assigns: %{state: %State{animate?: true}}} = scene) do
    Logger.debug(
  ...
  def handle_cast(:start_animation, %{assigns: %{state: %State{timer: nil} = state}} = scene) do
  ...
  def handle_input(
        {:cursor_pos, {x, y}},
        _context,
        %{assigns: %{frame: %Frame{} = frame, state: %State{cool_kid_radius: cool_kid_radius}}} =
          scene
      ) do
