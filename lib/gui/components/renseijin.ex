defmodule Flamelex.GUI.Component.Renseijin do
  ...
  @pi 3.14159265359
  # 100ms framerate? tick every 100ms?
  @animation_rate 10
  # area of effect for awesomeness to happen
  @cool_kid_radius 80
  ...
  def reset_mid_triangle(graph, scene) do
    ...
    |> Scenic.Graph.modify(
        :mid_triangle,
        &Scenic.Primitives.update_opts(&1, color: color, rotate: degree_in_radians(scene.assigns.rotation))
      )
  end
  ...
  def handle_info(:tick, %{assigns: %{rotation: r}} = scene) when r >= 0 and r <= 360 do
    ...
    scene = scene |> assign(rotation: r + 0.2)
  ...
  def draw_circles(graph, args) do
    ...
    |> Scenic.Primitives.circle(inner_circle_radius(args) + 1.5 * size,
      stroke: {1, @primary_color}
    )
  end
  ...
  def animate_mid_triangle(graph, rotation) do
    ...
    |> Scenic.Graph.modify(
      :mid_triangle,
      &Scenic.Primitives.update_opts(&1, stroke: {1, :green}, rotate: -1 * degree_in_radians(rotation))
    )
  end
