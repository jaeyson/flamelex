defmodule Memelex.GUI.Component.RapidSelector.Lens do # TODO rename this MicroFilm
  ...
  def handle_cast({:click, :new_tidbit}, scene) do
    Flamelex.Fluxus.action({RapidSelector, {:open_tidbit, {:new, %{mode: :edit}}}})
    {:noreply, scene}
  end
  ...
  def render_toolbar(graph, args) do
    ...
    |> Scenic.Primitives.group(
      fn graph ->
        ...
        |> Memelex.GUI.Components.IconButton.add_to_graph(
          %{
            frame: Frame.new(pin: {1 * 50, 0}, size: {50, 50}),
            icon: "ionicons/black_32/today.png"
          },
          id: :journal_today
        )
        |> Memelex.GUI.Components.IconButton.add_to_graph(
          %{
            frame: Frame.new(pin: {2 * 50, 0}, size: {50, 50}),
            icon: "ionicons/black_32/search.png"
          },
          id: :search_memex
        )
  ...
  def render_lower_pane(graph, %{
        frame: sidebar_frame,
        state: %{story_river: %{open_tidbits: open_tidbits}}
      }) do
  ...
  def render_open_tidbits(graph, sidebar_frame, tidbit_list) do
    {final_graph, _final_offset} =
      Enum.reduce(tidbit_list, {graph, 0}, fn tidbit, {graph, offset_count} ->
        # this is the height of each button, get it from somewhere better than hard-coded
        btn_height = 50
