defmodule Flamelex.GUI.Layers.Layer01 do
  ...
  alias Widgex.Frame
  alias Flamelex.GUI.Layers.Layer01.State
  require Logger
  ...
  def validate(%{frame: %Frame{}, state: %State{}} = data) do
    {:ok, data}
  end
  ...
  def init(
        %Scenic.Scene{} = scene,
        %{frame: %Frame{} = frame, state: %State{} = state},
        opts
      ) do
    ...
    # Flamelex.Lib.Utils.PubSub.subscribe(topic: @layer_1)
  ...
  def handle_info(
        ...
        %{assigns: %{frame: f, state: l1_state}} = scene
      ) do
  ...
  def handle_info(
    ...
    {:ok, %Scenic.Graph{} = new_graph} = render(f, new_l1_state)
  ...
  def render(frame, %{active_apps: []} = _layer_state) do
    ...
    graph =
      Scenic.Graph.build()
      |> Scenic.Primitives.rectangle(frame.size.box, fill: :blue, translate: frame.pin.point)

    {:ok, graph}
  end

  def render(frame, %{layout: :full_screen, active_apps: [app | _rest]}) do
